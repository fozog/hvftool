...
typedef struct sec_ram_vobject {
    vobject_t _;
    gpa_t start;
    gpa_t end;
    void* memory;
    bool is_sram;
    struct sec_ram_vobject* next;
} sec_ram_vobject_t;

static sec_ram_vobject_t* sec_ram_list = NULL;
static int sec_ram_list_count = 0;
...
static parameter_t parameters[] = {
	...
	.name = "address",
	.type = PARAM_UINT64,
	.description = "GPA of the secure ram being added",
	.is_mandatory = true,
	.u64_value = 0
    },
    {
...
static vobject_factory_t sec_ram_factory = {
    .key= "SECRAM",                                  // matching key for vobj selection
    .description = "SECRAM (backends: hostmem, map...)",      // description
    .fdt_default_name = "ram",                                  // default name for FDT
    .compatible = ONE_FDT_STRING("memory"),               // default FDT compatiblee
    .size = sizeof(sec_ram_vobject_t),                  // size of the associated vobject
...
static vobject_t* initialize(struct vmm_context* context, vobject_t* vobj)
{
    ...
    sec_ram_vobject_t* ramvobj = (sec_ram_vobject_t*)vobj;
    ...
    param = parameter_lookup(vobj->parameters, vobj->parameters_count, "sram");
    is_sram = param != NULL ? param->bool_value : false;
    
    //free(param);
    
    size_t capacity = mem_backend->get_capacity(mem_backend);
    ramvobj->start = address;
    ramvobj->is_sram = is_sram;
    ...
    gpa_t top = hvftool_config.memory_map->sec_ram_start + capacity;
    if (top > hvftool_config.memory_map->sec_ram_top) {
	hvftool_config.memory_map->sec_ram_top = top;
	hvftool_config.memory_map->sec_ram_top_free = hvftool_config.memory_map->sec_ram_top;
	TRACE(DEBUG_RAM, "RAM: adjusting top and top_free to %llx\n", hvftool_config.memory_map->sec_ram_top);
    }
    ...
    //Add front
    ramvobj->next = sec_ram_list;
    sec_ram_list = ramvobj;
    sec_ram_list_count++;
...
static void fdt_generator(struct vobject* vobj, void* fdt)
{
    ...
    char node_name[32];
    sprintf(node_name, "secram@%llx", address); // this is to avoid dtc complaining "node has a reg or ranges property, but no unit name"
...
int sec_ram_init(void)
{
    return vobjtype_register(&sec_ram_factory);
}
